...
\begin{minipage}{1\linewidth}
  ...
  \begin{algorithmic}[1]
    ...
    \State $ek' \gets$ \Call{\texttt{PKE.UpdEk}}{$ek', T_\t{S}[i]$}
    \EndFor
    \State $c' \gets$ \Call{\texttt{PKE.Enc}}{$ek',l,m,T_\t{S}$}
    ...
    \State $vk'' \gets$ \Call{\texttt{DS.UpdVk}}{$vk'', T_\t{S}[i]$}
    \EndFor
    \State \textbf{assert} \Call{\texttt{DS.Verify}}{$vk'',\sigma,v,T_\t{S}$}
    ...
    \For{$i = r^\t{ack}$ to $s$}
    \State $dk[i] \gets$ \Call{\texttt{PKE.UpdDk}}{$dk[i],T_\t{R}$}
    \EndFor
    \State $st \gets (s,r,r^\t{ack},sk,vk',ek',dk,hk,T_\t{R},T_\t{S})$
    \State \Return $(st,m)$
    \EndProcedure
